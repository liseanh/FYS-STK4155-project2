...
\twocolumn[
  \begin{@twocolumnfalse}
    ...
    \begin{abstract}
		...
		results superior to those of the logistic regression, with area ratios of
		$0.55$ and $0.43$ respectively, while we for regression found that the
		neural network is excessively costly in comparison to more standard linear
		regression methods with respect to performance and tuning, with OLS
		giving $R^2=0.82$ and MLP giving $R^2=0.59$ for 400 data points.
	\end{abstract}
  \end{@twocolumnfalse}
]
...
For the classification part of this project, we will use credit card payment
...
non-default samples. This was done to avoid our optimizer favoring a model that
predicts the non-default outcome in all cases. Note that we did not upsample the
amount of defaults in the test set.
...
$\bm{\hat{y}}$ fits the true values $\bm{y}$, with a value of 1 indicating a
...
\begin{figure}
	...
	regression for various values of the learning rate $\eta$ chosen using
  randomized search.}
...
non-default. Both area ratios can be found in Table \ref{tab:area_ratios}.
...
\begin{table}[H]
    ...
    \hline
		Method & \makecell[l]{Error \\rate} & \makecell[l]{Area ratio \\default} 
		& \makecell[l]{Area ratio \\non-default} \\ \hline
