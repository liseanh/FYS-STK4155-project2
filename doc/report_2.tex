...
previous project and compare the $R^2$ score.
...
assigned to each feature in $X$.  The cost function most commonly used in this
...
technique as in the article used for comparison: The area ratio. This is the area
between the baseline and our model in a cumulative gains plot divided by the area
between the theoretically best model and the baseline. To generate cumulative gains
plots, we used the Python library \texttt{Scikit-Plot}'s function
\texttt{plot_cumulative_gain}. How these plots are interpreted is explained 
