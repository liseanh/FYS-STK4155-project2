...
\subsection{Classification}
Figure \ref{fig:logreg_gain} shows the cumulative gains plot for the logistic 
regression using the learning rate in Table \ref{tab:hyperparameters} and the 
other hyperparameters as described in \ref{subsec:tuning_hyperparameters}. From 
the figure, we can see that our LR model performs worse when predicting non-default 
outcomes than default. Both area ratios can be found in Table 
\ref{tab:area_ratios}. 

\begin{figure}[H]
	\includegraphics[scale=1]{figures/cumulative_gain_logreg.pdf}
	\caption{Cumulative gains plot for logistic regression fit on the credit card data.
...
\begin{figure}[H]
	...
	\caption{Cumulative gains plot for the MLP fit on the credit card data.
  The closer a curve is to the "best model" curve, the better the model is at
  predicting that given outcome. Our model is much better at predicting default
  than non-default.}
	\label{fig:nn_gain}
...
\ref{fig:cumulative_gain_logreg} and \ref{fig:cumulative_gain_NN} align closely
...
Regression
%* NN slightly better, but so would OLS be with more data? OLS much better on n=20
%* NN no polynomial degrees, saves memory, smaller feature matrix
