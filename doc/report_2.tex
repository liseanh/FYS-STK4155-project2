...
We use \texttt{Scikit-learn's}\cite{sklearn_api} library  to split the data
sets into test and training sets, and to scale and center the data.
...
predictors in the test set were subtracted from both the training and the test
sets. The data is scaled by normalizing with respect to the variances of the
...
find better hyperparameters  \ref{bergstra2012random}. For each trial parameter,
...
as that is suitable for very many types of data \cite{ML_algo}. Our first hidden layer has 100
nodes, the second has 50. This leaves learning rate and the regularization parameter
which are found using randomized search.

To perform the parameter search we use the RandomizedSearchCV class provided
by Scikit-Learn \ref{sklearn_api} on hundred samples, with 5-fold cross validation
to evaluate each sample's accuracy score.
